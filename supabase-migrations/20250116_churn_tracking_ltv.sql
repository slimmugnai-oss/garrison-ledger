...
CREATE TABLE IF NOT EXISTS public.user_activity_log (
    ...
    activity_type TEXT NOT NULL, -- login, plan_generated, tool_used, assessment_completed, etc.
    activity_details JSONB, -- Additional context (which tool, etc.)
...
CREATE TABLE IF NOT EXISTS public.subscription_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id TEXT NOT NULL,
    event_type TEXT NOT NULL, -- subscribed, cancelled, reactivated, upgraded, downgraded
    previous_status TEXT,
    new_status TEXT,
    reason TEXT, -- User-provided cancellation reason
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS public.user_ltv_metrics (
    ...
    total_spent_cents INTEGER NOT NULL DEFAULT 0, -- Total revenue from user
    lifetime_days INTEGER NOT NULL DEFAULT 0, -- Days since account creation
    subscription_start_date TIMESTAMPTZ,
    subscription_end_date TIMESTAMPTZ,
    churn_risk_score INTEGER NOT NULL DEFAULT 0, -- 0-100 (100 = highest risk)
...
BEGIN
    ...
    -- Update LTV metrics based on activity
...
BEGIN
    ...
    -- Scoring algorithm (0-100, higher = more risk)
    -- Last login
...
CREATE OR REPLACE FUNCTION get_cohort_retention(
    p_cohort_month TEXT -- Format: 'YYYY-MM'
)
RETURNS TABLE (
    cohort_month TEXT,
    total_users INTEGER,
...
BEGIN
    ...
    -- Calculate retention (users who logged in after X days)
    RETURN QUERY
...
BEGIN
    ...
    FROM public.user_ltv_metrics
    WHERE total_spent_cents > 0; -- Only paying customers
END;
$$ LANGUAGE plpgsql;
...
-- Users can read their own LTV metrics
CREATE POLICY "Users can read their own LTV metrics"
    ON public.user_ltv_metrics FOR SELECT
    USING (auth.uid() = user_id);

-- Service role has full access (for admin and analytics)
...
RETURNS TRIGGER AS $$
BEGIN
    -- Recalculate churn risk whenever activity is logged
