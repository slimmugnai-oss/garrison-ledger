...
export function runPlanBuckets(facts: AssessmentFacts): PlanBuckets {
  ...
  if (topicInterests.includes('tsp') && knowledgeLevel === 'beginner') buckets.finance.push({ slug: 'the-two-pillars-pension-and-tsp', why: 'Because you want to understand TSP, this breaks down the BRS in plain language.' });
  if (topicInterests.includes('sdp')) buckets.finance.push({ slug: 'lower-your-interest-rates-with-scra', why: 'Because SDP and SCRA both maximize deployment pay, pairing them protects your financial future.' });
