import type { UserContext } from './personalize';

export type DoNowItem = { id: string; text: string };
export type EstImpact = { label: string; value: string };

export function whyItMattersBullets(ctx: UserContext): string[] {
  const out: string[] = [];
  const deps = Number(ctx.dependents || 0);
  const bah = Number(ctx.bah || 0);
  const pcsSoon = Number(ctx.timelineMonths || 24) <= 6;
