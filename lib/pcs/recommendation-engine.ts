...
export function generateStaticRecommendations(claimData: ClaimData): Recommendation[] {
  ...
  if (claimData.move_type === "dity" || claimData.move_type === "partial") {
    recommendations.push({
      ...
      message:
        "For DITY moves, keep EVERY receipt for gas, tolls, hotels, meals, truck rental, packing materials, and equipment. These are 100% reimbursable and significantly increase your payout.",
  ...
  if (claimData.documents_uploaded) {
    ...
    if (!docs.orders) {
      recommendations.push({
        ...
        message:
          "You must upload your official PCS orders. This is the #1 most important document for your claim.",
    ...
    if (claimData.move_type === "dity" && !docs.weigh_ticket) {
      recommendations.push({
        ...
        message:
          "DITY moves require both EMPTY and FULL weigh tickets. Visit a certified weigh station (CAT scales) before loading and after loading your truck.",
  ...
  if (
    claimData.estimated_weight &&
    claimData.estimated_weight >= 3000 &&
    claimData.estimated_weight <= 8000
  ) {
    recommendations.push({
      id: "dity-sweet-spot",
      type: "cost_savings",
  ...
  if (claimData.rank && (claimData.rank.startsWith("E-7") || claimData.rank.startsWith("O-"))) {
    recommendations.push({
      ...
      message:
        "As an E-7+ or Officer, you may be authorized a house hunting trip (HHT) to your new duty station. Check with your TMO office.",
  ...
  if (claimData.departure_date) {
    recommendations.push({
      ...
      message:
        "If you're PCSing after a deployment, remember to withdraw your SDP funds (if applicable) before moving to maximize the 10% annual return.",
  ...
  if (claimData.estimated_weight && claimData.estimated_weight > 8000) {
    recommendations.push({
      ...
      message:
        "With a larger shipment, you're authorized up to 90 days of Storage in Transit (SIT) if you can't immediately move into your new residence. Plan ahead.",
  ...
  if (claimData.distance_miles && claimData.distance_miles > 1000) {
    recommendations.push({
      ...
      message:
        "Long-distance PCS moves may qualify for an Advance Operating Allowance (AOP) to help with upfront costs. Contact your finance office.",
...
function getWeightAllowance(rank: string, hasDependents: boolean): number {
  ...
  const allowances: Record<string, { solo: number; deps: number }> = {
    ...
    "W-1": { solo: 10000, deps: 12000 },
    "W-2": { solo: 11000, deps: 13000 },
    "W-3": { solo: 12000, deps: 14000 },
    "W-4": { solo: 13000, deps: 15000 },
    "W-5": { solo: 13000, deps: 16000 },
    // Officers (O1-O10)
    ...
    "O-7": { solo: 15000, deps: 18000 },
    "O-8": { solo: 16000, deps: 18000 },
    "O-9": { solo: 17000, deps: 18000 },
    "O-10": { solo: 18000, deps: 18000 },
  };
