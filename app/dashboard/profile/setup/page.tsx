...
export default function ProfileSetupPage() {
  ...
  // Get referrer from query params (for context-aware navigation)
  const referrer = searchParams?.get("from") || "dashboard";
  const backUrl = referrer === "settings" ? "/dashboard/settings" : "/dashboard";
  const backLabel = referrer === "settings" ? "Settings" : "Dashboard";
  ...
  const getSectionCompletion = (
    ...
    switch (section) {
      ...
      case 2: // Military identity
        total =
          data.service_status && !["military_spouse", "dod_civilian"].includes(data.service_status)
            ? 3
            : 1;
        if (data.service_status) complete++;
        if (
          data.service_status &&
          !["military_spouse", "dod_civilian"].includes(data.service_status)
        ) {
  ...
  const overallProgress = useMemo(() => {
    ...
    return {
      ...
      total: totalFields,
      percentage: (totalComplete / totalFields) * 100,
    };
  ...
  const tabs: Array<{
    id: TabId;
    label: string;
    icon: "User" | "Shield" | "DollarSign" | "File";
    sections: number[];
  }> = [
  ...
  const getTabCompletion = (sections: number[]) => {
    ...
    return {
      complete: totalComplete,
      total: totalFields,
      percentage: totalFields > 0 ? (totalComplete / totalFields) * 100 : 100,
    };
  };
  ...
  async function submit() {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      setError(
        `Please complete the following required fields: ${Object.values(errors).join(", ")}`
      );
    ...
    try {
      ...
      // Auto-redirect to referrer (context-aware) after 1.5 seconds
      setTimeout(() => {
        router.push(backUrl);
      }, 1500);
    } catch (err) {
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-slate-700 to-slate-900 text-2xl text-white shadow-lg">
              👤
            </div>
            <div>
              <h1 className="text-3xl font-bold text-primary">Set up your profile</h1>
              <p className="text-body text-sm">Help us personalize your experience</p>
            </div>
          </div>
          <Link
            href={backUrl}
            className="text-info hover:text-info hidden text-sm transition-colors hover:underline md:block"
          >
            Skip for now
          </Link>
        </div>
        ...
        {/* Error Messages */}
        {error && (
          <div className="bg-danger-subtle mb-6 rounded-r-lg border-l-4 border-danger p-4">
        ...
        {/* Success Message */}
        {saved && (
          <div className="bg-success-subtle mb-6 rounded-r-lg border-l-4 border-success p-4">
            <div className="flex items-center gap-3">
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <Icon
                    name={tab.icon}
                    className={`h-5 w-5 ${activeTab === tab.id ? "text-blue-600" : "text-gray-400"}`}
                  />
                  <span>{tab.label}</span>
          ...
          {activeTab === "military" && (
            <>
              ...
              >
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    >
                      <select
                        className={getInputClass(
                          !!fieldErrors.service_status,
                          !!data.service_status
                        )}
                        value={data.service_status ?? ""}
                        onChange={(e) =>
                          setData((d) => ({ ...d, service_status: e.target.value || null }))
                        }
                      >
                        <option value="">Select</option>
                    ...
                    {data.service_status === "military_spouse" ? (
                      <>
                        <div className="flex items-center justify-center text-sm italic text-muted md:col-span-2">
                      ...
                      <>
                        ...
                        >
                          <select
                            ...
                            value={data.branch ?? ""}
                            onChange={(e) =>
                              setData((d) => ({ ...d, branch: e.target.value || null }))
                            }
                          >
                            <option value="">Select (optional)</option>
                            {civilianBranches.map((b) => (
                        ...
                        <div className="flex items-center justify-center text-sm italic text-muted md:col-span-1">
                          Rank not applicable
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        >
                          <select
                            ...
                            value={data.rank ?? ""}
                            onChange={(e) =>
                              setData((d) => ({ ...d, rank: e.target.value || null }))
                            }
                            disabled={!data.branch}
                          >
                            <option value="">
                              Select {!data.branch ? "(select branch first)" : ""}
                            </option>
                            {availableRanks.length > 0 && (
                              <>
                                {ranksData[data.branch!]?.enlisted &&
                                  ranksData[data.branch!].enlisted!.length > 0 && (
                                    <optgroup label="Enlisted">
                                      {ranksData[data.branch!].enlisted!.map((r) => (
                                        <option key={r.code + "-" + r.title} value={r.title}>
                                          {r.title}
                                        </option>
                                      ))}
                                    </optgroup>
                                  )}
                                {ranksData[data.branch!]?.warrant &&
                                  ranksData[data.branch!].warrant!.length > 0 && (
                                ...
                                {ranksData[data.branch!]?.officer &&
                                  ranksData[data.branch!].officer!.length > 0 && (
                ...
                {data.service_status &&
                  ["reserve", "national_guard"].includes(data.service_status) && (
                    <div className="mt-4">
                      <p className="mb-2 text-xs text-muted">
                        🎖️ You selected{" "}
                        {serviceStatuses.find((s) => s.value === data.service_status)?.label}. This
                        gives us context for your service type.
                      </p>
                    </div>
                  )}
                {data.service_status === "retired" && (
                  <div className="bg-info-subtle border-info mt-4 rounded-r-lg border-l-4 p-4">
                    <p className="text-sm text-blue-900">
                      <strong>Thank you for your service!</strong> We&apos;ll tailor content for
                      your retirement phase, including continued benefits, VA resources, and
                      transition planning.
                    </p>
                  </div>
                )}
                {data.service_status === "veteran" && (
                  <div className="bg-success-subtle mt-4 rounded-r-lg border-l-4 border-success p-4">
                    <p className="text-sm text-success">
                      <strong>Thank you for your service!</strong> We&apos;ll focus on veteran
                      benefits, civilian career transition, and post-service life planning.
                    </p>
                  </div>
                )}
                {data.service_status === "separating" && (
                  <div className="mt-4 rounded-r-lg border-l-4 border-amber-500 bg-amber-50 p-4">
                    <p className="text-sm text-amber-900">
                      <strong>Transition Planning Mode:</strong> We&apos;ll prioritize TAP/SFL-TAP
                      resources, resume building, job search strategies, and benefits preservation.
                    </p>
                  </div>
                )}
                {data.service_status === "military_spouse" && (
                  <div className="mt-4 rounded-r-lg border-l-4 border-purple-500 bg-purple-50 p-4">
                    <p className="text-sm text-purple-900">
                      <strong>Military Spouse Resources:</strong> We&apos;ll focus on spouse
                      employment, PCS support, deployment readiness, and comprehensive planning for
                      military family life.
                    </p>
                  </div>
                )}
                {data.service_status === "dod_civilian" && (
                  <div className="mt-4 rounded-r-lg border-l-4 border-indigo-500 bg-indigo-50 p-4">
                    <p className="text-sm text-indigo-900">
                      <strong>DoD Civilian Resources:</strong> We&apos;ll focus on federal
                      employment benefits, TSP optimization, and career advancement within the DoD
                      system.
                    </p>
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    >
                      <select
                        ...
                        onChange={(e) =>
                          setData((d) => ({ ...d, marital_status: e.target.value || null }))
                        }
                      >
                        <option value="">Select</option>
                    ...
                    >
                      <input
                        ...
                        placeholder="0"
                        className={getInputClass(
                          !!fieldErrors.num_children,
                          data.num_children !== null && data.num_children !== undefined
                        )}
                        value={data.num_children ?? 0}
                        onChange={(e) =>
                          setData((d) => ({ ...d, num_children: Number(e.target.value) }))
                        }
                      />
                    </ProfileFormField>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    >
                      <select
                        className={getInputClass(
                          !!fieldErrors.tsp_balance_range,
                          !!data.tsp_balance_range
                        )}
                        value={data.tsp_balance_range ?? ""}
                        onChange={(e) =>
                          setData((d) => ({ ...d, tsp_balance_range: e.target.value || null }))
                        }
                      >
                        <option value="">Select</option>
                        {tspRanges.map((r) => (
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    >
                      <select
                        ...
                        onChange={(e) =>
                          setData((d) => ({ ...d, housing_situation: e.target.value || null }))
                        }
                      >
                        <option value="">Select (optional)</option>
                        {housingOptions.map((h) => (
                          <option key={h} value={h.toLowerCase().replace(" ", "-")}>
                            {h}
                          </option>
                        ))}
                      </select>
                    ...
                    <ProfileFormField
                      ...
                      success={
                        data.owns_rental_properties !== null &&
                        data.owns_rental_properties !== undefined
                      }
                    >
                      <select
                        className={getInputClass(
                        ...
                        value={
                          ...
                          data.owns_rental_properties === undefined
                            ? ""
                            : String(data.owns_rental_properties)
                        }
                        onChange={(e) =>
                          setData((d) => ({
                            ...d,
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setData((d) => ({
                            ...d,
                            retirement_age_target: e.target.value ? Number(e.target.value) : null,
                          }))
                        }
                      />
                    </ProfileFormField>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="mt-1 text-sm text-yellow-800">
                            We couldn't auto-detect the Military Housing Area (MHA) code for{" "}
                            {data.current_base}. You can enter it manually below to enable accurate
                            BAH verification.
                          </p>
                          <ProfileFormField
                          ...
                          >
                            <input
                              ...
                              onChange={(e) =>
                                setData((d) => ({
                                  ...d,
                                  mha_code_override: e.target.value.toUpperCase() || null,
                                }))
                              }
                              maxLength={10}
                            />
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-blue-800">
                          The LES Auditor can verify your special pays are correct if you configure
                          them here. This is completely optional - skip if you don't receive any
                          special pays.
                        </p>
                      </div>
                    </div>
                      ...
                      >
                        <div className="relative">
                          ...
                          <input
                            ...
                            value={
                              data.sdap_monthly_cents
                                ? (data.sdap_monthly_cents / 100).toString()
                                : ""
                            }
                            onChange={(e) => {
                              ...
                              setData((d) => ({
                                ...d,
                                sdap_monthly_cents:
                                  value && !isNaN(num) ? Math.round(num * 100) : null,
                              }));
                            }}
                        ...
                        <label className="flex cursor-pointer items-center gap-2">
                          <input
                            ...
                            onChange={() =>
                              setData((d) => ({
                                ...d,
                                receives_hfp_idp: true,
                                hfp_idp_monthly_cents: 22500,
                              }))
                            }
                            className="h-4 w-4 text-blue-600"
                          />
                        ...
                        <label className="flex cursor-pointer items-center gap-2">
                          <input
                            ...
                            onChange={() =>
                              setData((d) => ({
                                ...d,
                                receives_hfp_idp: false,
                                hfp_idp_monthly_cents: null,
                              }))
                            }
                            className="h-4 w-4 text-blue-600"
                          />
                      ...
                      >
                        <div className="relative">
                          ...
                          <input
                            ...
                            value={
                              data.flpp_monthly_cents
                                ? (data.flpp_monthly_cents / 100).toString()
                                : ""
                            }
                            onChange={(e) => {
                              const value = e.target.value.replace(/[^0-9.]/g, "");
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-lg border border-blue-300 bg-blue-50 p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-blue-800">
                          Configure your deductions and tax settings to enable full LES validation
                          including net pay verification. This helps catch errors in TSP
                          contributions, SGLI premiums, and tax withholding.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <div className="relative">
                        <input
                          ...
                          value={
                            data.tsp_contribution_percent != null
                              ? Math.round(data.tsp_contribution_percent * 100)
                              : ""
                          }
                          onChange={(e) =>
                            setData((d) => ({
                              ...d,
                              tsp_contribution_percent: e.target.value
                    ...
                    >
                      <select
                        ...
                        onChange={(e) =>
                          setData((d) => ({ ...d, tsp_contribution_type: e.target.value || null }))
                        }
                      >
                        <option value="">Select</option>
                    ...
                    >
                      <select
                        ...
                        onChange={(e) =>
                          setData((d) => ({
                            ...d,
                            sgli_coverage_amount: e.target.value ? parseInt(e.target.value) : null,
                          }))
                        }
                      >
                        <option value="">Select</option>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <select
                        ...
                        onChange={(e) =>
                          setData((d) => ({ ...d, filing_status: e.target.value || null }))
                        }
                      >
                        <option value="">Select</option>
                        <option value="single">Single</option>
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setData((d) => ({
                            ...d,
                            state_of_residence: e.target.value.toUpperCase() || null,
                          }))
                        }
                      />
                    </ProfileFormField>
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setData((d) => ({
                            ...d,
                            w4_allowances: e.target.value ? parseInt(e.target.value) : null,
                          }))
                        }
                      />
                    </ProfileFormField>
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <p className="text-xs text-gray-700">
                      <Icon name="HelpCircle" className="mr-1 inline h-3 w-3" />
                      <strong>Why configure this?</strong> The LES Auditor can calculate your
                      expected deductions and taxes to verify your net pay is correct. Tax
                      calculations are estimates - actual withholding depends on your W-4 settings
        ...
        <div className="bg-surface border-subtle fixed bottom-0 left-0 right-0 z-50 border-t p-4 shadow-lg md:hidden">
          <button
            onClick={submit}
            disabled={saving || saved}
            className="inline-flex w-full items-center justify-center rounded-xl bg-gradient-to-r from-slate-900 to-slate-800 px-6 py-4 font-bold text-white shadow-lg transition-all hover:from-slate-800 hover:to-slate-900 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {saving ? (
              <>
                <svg
        ...
        {/* Desktop Save Button */}
        <div className="mb-20 mt-8 hidden items-center gap-4 md:flex">
          <button
            onClick={submit}
            disabled={saving || saved}
            className="inline-flex transform items-center rounded-xl bg-gradient-to-r from-slate-900 to-slate-800 px-8 py-4 font-bold text-white shadow-lg transition-all hover:scale-105 hover:from-slate-800 hover:to-slate-900 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
          >
            {saving ? (
              <>
                <svg
          ...
          {!saved && (
            <Link
              href={backUrl}
              className="text-body text-sm transition-colors hover:text-primary hover:underline"
            >
