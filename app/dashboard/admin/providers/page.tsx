      ...
      <>
        ...
        <div className="p-6 max-w-4xl mx-auto">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
            ...
            <p className="text-red-700">You are not authorized to manage providers. Contact the administrator if you believe this is an error.</p>
          </div>
        </div>
      </>
    );
                ...
                <div key={r.id} className="border rounded p-3">
                  <div className="flex justify-between">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{r.type} · {r.city || ""}{r.city && r.state ? ", " : ""}{r.state || ""}</div>
                    </div>
                  </div>
                  <div className="mt-2 flex gap-2">
                    <button onClick={() => approve(r.id)} className="text-xs rounded border px-2 py-1 hover:bg-green-50">Approve</button>
                  </div>
                </div>
              ))}
            </div>
