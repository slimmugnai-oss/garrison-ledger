  ...
  try {
    ...
    try {
      ...
      if (creditBalance && creditBalance > 0) {
        ...
        await supabaseAdmin.from("user_reward_credits").insert({
          user_id: user.id,
    ...
    // Validate price ID exists and determine checkout mode
    ...
    try {
      ...
      // Determine mode: subscription (premium) vs payment (question packs)
      // Subscription price IDs start with price_1SHd (premium monthly/annual)
      // Question pack price IDs start with price_1SLS (one-time purchases)
      checkoutMode = priceId.startsWith("price_1SHd") ? "subscription" : "payment";

      logger.debug("Checkout mode determined", {
        priceId,
        mode: checkoutMode,
      });
    } catch (priceError) {
      logger.error("Invalid Stripe price ID", priceError, { priceId });
      throw Errors.invalidInput("Invalid price ID", {
        priceId,
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url:
        successUrl ||
        `${process.env.NEXT_PUBLIC_SITE_URL || "https://www.garrisonledger.com"}/dashboard/upgrade/success`,
      cancel_url:
        cancelUrl ||
        `${process.env.NEXT_PUBLIC_SITE_URL || "https://www.garrisonledger.com"}/dashboard/upgrade?canceled=true`,
      metadata: {
        userId: user.id,
        checkoutMode, // Store mode for webhook processing
      },
      // 🎯 AUTO-APPLY REFERRAL CREDIT DISCOUNT (only for subscriptions)
      ...(discountCouponId &&
