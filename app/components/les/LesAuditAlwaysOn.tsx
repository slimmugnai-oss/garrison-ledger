      ...
      <div className="overflow-y-auto p-6 bg-gray-50">
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-lg border">
            <button
              onClick={() => setCollapsedSections(prev => ({ ...prev, entitlements: !prev.entitlements }))}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <Badge variant="info">{[basePay, bah, bas, cola].filter(v => v > 0).length}/4</Badge>
              </div>
              <Icon 
          ...
          <div className="bg-white rounded-lg border">
            <button
              onClick={() => setCollapsedSections(prev => ({ ...prev, deductions: !prev.deductions }))}
              className="w-full px-4 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors"
            >
              <div className="flex items-center gap-3">
          ...
          <div className="bg-white rounded-lg border">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <Badge variant="info">{[federalTax, stateTax, fica, medicare].filter(v => v > 0).length}/4</Badge>
              </div>
              <Icon 
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {loading && (
            <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-lg">
              <Icon name="Loader" className="h-5 w-5 text-blue-600 animate-spin" />
            ...
            <>
              <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* Expected Net */}
                  <div>
                    <p className="text-sm text-blue-700 font-medium mb-1">Expected Net Pay</p>
                  ...
                  {/* Actual Net */}
                  <div>
                    <p className="text-sm text-gray-700 font-medium mb-1">Your LES Net Pay</p>
                    {result.totals.actual_net > 0 ? (
                      <p className="text-2xl font-bold text-gray-900">
                        ${(result.totals.actual_net / 100).toFixed(2)}
                      </p>
                    ) : (
                      <p className="text-sm text-gray-500 italic">Enter above to compare</p>
                    )}
                  </div>

                  {/* Variance */}
                  <div>
                    <p className="text-sm text-gray-700 font-medium mb-1">Variance</p>
                    {result.totals.variance !== null ? (
                      <p className={`text-2xl font-bold ${
                        Math.abs(result.totals.variance) <= 500 ? 'text-green-600' :
                        Math.abs(result.totals.variance) <= 5000 ? 'text-yellow-600' :
                        'text-red-600'
                      }`}>
                      ...
                      <Badge variant={
                        result.totals.varianceBucket === '0-5' ? 'success' :
                        result.totals.varianceBucket === '5-50' ? 'warning' :
                        'danger'
                      }>
                ...
                <div className="mt-4">
                  <Badge 
                    variant={
                      ...
                      result.confidence === 'medium' ? 'warning' :
                      'danger'
                    }
                  >
              ...
              {/* Flags List */}
              <div>
                <h3 className="font-semibold text-lg text-gray-900 mb-3">Findings</h3>
                  ...
                  {result.flags.map((flag, idx) => (
                    <div 
                      ...
                      className={`p-4 rounded-lg border-l-4 ${
                        flag.severity === 'red' ? 'bg-red-50 border-red-500' :
                        flag.severity === 'yellow' ? 'bg-yellow-50 border-yellow-500' :
                        'bg-green-50 border-green-500'
                      }`}
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <p className="font-semibold text-gray-900 mb-1">{flag.message}</p>
              ...
              <PremiumCurtain tier={tier} feature="waterfall">
                <div>
                  <h3 className="font-semibold text-lg text-gray-900 mb-3">Detailed Reconciliation</h3>
              ...
              <div className="flex items-center gap-4 pt-6 border-t">
                {tier === 'premium' ? (
                  <button
                    ...
                    disabled={saving}
                    className="flex-1 inline-flex items-center justify-center gap-2 px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors"
                  >
          ...
          {!result && !loading && (
            <div className="text-center py-12">
              <Icon name="FileText" className="h-16 w-16 text-gray-300 mx-auto mb-4" />
              <p className="text-gray-600">Enter your LES data on the left to see your audit report</p>
            </div>
          )}
        </div>
      </div>
