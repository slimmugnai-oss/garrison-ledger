...
export default function StorageBar({ used, limit, isPremium: _isPremium }: StorageBarProps) {
  ...
  return (
    <div className="mb-6 rounded-xl border border-[#2A2F3E] bg-gradient-to-br from-[#1A1F2E] to-[#141824] p-6 shadow-lg">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="text-right">
          <span className="text-lg font-bold text-white">{formatFileSize(used)}</span>
          <span className="text-sm text-muted"> / {formatFileSize(limit)}</span>
          {upgradeText && (
            <a
              href="/dashboard/upgrade"
              className="mt-1 block text-xs font-medium text-[#00E5A0] transition-colors hover:text-[#00CC8E]"
            >
              {upgradeText}
            </a>
          )}
      ...
      {/* Progress Bar */}
      <div className="relative h-3 w-full overflow-hidden rounded-full bg-[#0A0F1E] shadow-inner">
        <div
