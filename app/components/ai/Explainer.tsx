...
export default function Explainer({ payload }: { payload: Record<string, unknown> }) {
  ...
  const isTruncated =
    !isPremium && fullText && fullText.split(/[.!?]+/).filter((s) => s.trim()).length > 3;
  ...
  return (
    <div className="mt-6">
      <button
        onClick={run}
        disabled={loading}
        className="rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md transition-all duration-200 hover:from-indigo-700 hover:to-purple-700 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50"
      >
      ...
      {text && (
        <div className="mt-4">
          <div
            className="text-body rounded-lg border-2 border-indigo-100 bg-gradient-to-br from-white to-indigo-50/30 p-5 text-sm leading-relaxed shadow-md"
            ...
            <div className="mt-4 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="mb-2 text-lg font-bold">Upgrade to Read Full AI Analysis</h3>
                  <p className="mb-4 text-white/90">
                    This is just a preview. Premium members get complete AI explanations for all
                    calculator results, plus:
                  </p>
                  <ul className="mb-4 space-y-2 text-sm text-white/95">
                  ...
                  <Link
                    ...
                    className="bg-surface text-info hover:bg-info-subtle inline-block rounded-lg px-6 py-3 font-bold shadow-lg transition-colors"
                  >
