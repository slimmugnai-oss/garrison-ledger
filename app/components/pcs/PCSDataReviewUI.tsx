...
export default function PCSDataReviewUI({
  ...
  const getConfidenceBadge = (status: string, confidence: number) => {
    if (status === 'excellent') {
      return <Badge variant="success" className="text-xs">✓ {confidence}%</Badge>;
    } else if (status === 'good') {
      return <Badge variant="warning" className="text-xs">~ {confidence}%</Badge>;
    } else if (status === 'needs_review') {
      return <Badge variant="danger" className="text-xs">! {confidence}%</Badge>;
    } else {
      return <Badge variant="secondary" className="text-xs">Missing</Badge>;
    }
  };
  ...
  const getCriticalFields = () => {
    return Object.entries(fieldScores)
      .filter(([_, score]) => score.status === 'needs_review' || score.status === 'missing')
      .length;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={overallConfidence >= 90 ? "border-green-300 bg-green-50" : overallConfidence >= 70 ? "border-yellow-300 bg-yellow-50" : "border-red-300 bg-red-50"}>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-slate-600 mt-2">
            Fields marked with <Badge variant="success" className="inline-flex text-xs mx-1">✓</Badge> were extracted with high confidence.
            Fields marked with <Badge variant="danger" className="inline-flex text-xs mx-1">!</Badge> should be verified carefully.
          </p>
        </CardHeader>
              ...
              >
                ...
                {fieldKey === 'ppm_authorized' ? (
                  ...
                  <input
                    ...
                    placeholder={fieldScore?.status === 'missing' ? 'Not found - please enter manually' : ''}
                  />
                ) : (
                  <Input
                    type={fieldKey.includes('dependents') || fieldKey.includes('weight') ? 'number' : 'text'}
                    value={value || ''}
                    onChange={(newValue: string) => handleFieldChange(fieldKey, newValue)}
                    className={fieldScore?.status === 'needs_review' || fieldScore?.status === 'missing'
                      ? 'border-red-500 focus:ring-red-500'
                      : ''}
                    placeholder={fieldScore?.status === 'missing' ? 'Not found - please enter manually' : ''}
                  />
                ...
                {fieldScore && fieldScore.status !== 'excellent' && (
                  <div className="mt-2 text-xs text-slate-600">
                    {fieldScore.status === 'missing' && '⚠️ This field was not found in your orders - please enter it manually'}
                    {fieldScore.status === 'needs_review' && '⚠️ Low confidence extraction - please verify this value carefully'}
